...
module.exports = grammar({
  ...
  rules: {
    ...
    import_spec_list: ($) =>
      seq('(', repeat(seq($.import_spec, terminator)), ')'),
    ...
    type_parameter_list: ($) =>
      seq('[', commaSep1($.parameter_declaration), optional(','), ']'),
    ...
    type_arguments: ($) =>
      prec.dynamic(2, seq('[', commaSep1($._type), optional(','), ']')),

    pointer_type: ($) => prec(PREC.unary, seq('*', $._type)),

    array_type: ($) =>
      seq('[', field('length', $._expression), ']', field('element', $._type)),
    ...
    _interface_body: ($) =>
      choice($.method_spec, $.interface_type_name, $.constraint_elem),

    interface_type_name: ($) => choice($._type_identifier, $.qualified_type),

    constraint_elem: ($) =>
      seq($.constraint_term, repeat(seq('|', $.constraint_term))),

    constraint_term: ($) => prec(-1, seq(optional('~'), $._type_identifier)),
    ...
    special_argument_list: ($) =>
      seq('(', $._type, repeat(seq(',', $._expression)), optional(','), ')'),
    ...
    format_verb: ($) => token.immediate(seq('%', choice(/[a-x,A-X,%]/))),
